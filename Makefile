...
# if .transifexrc does not exist yet, create it for read only access
$(HOME)/.transifexrc:
	echo "[https://www.transifex.com]" > $@
	echo "hostname = https://www.transifex.com" >> $@
	echo "username = c2corg_i18n" >> $@
	echo "password = c2corg_i18n" >> $@
	echo "token =" >> $@

.build/locale/c2corg_ui-client.pot: $(APP_HTML_FILES) $(APP_PARTIAL_FILES) $(APP_JS_FILES)
	mkdir -p $(dir $@)
...
transifex-send: .tx/config .build/locale/c2corg_ui-client.pot
	.build/venv/bin/tx push --source

.PHONY: transifex-init
transifex-init: .build/dev-requirements.timestamp .tx/config .build/locale/c2corg_ui-client.pot
	.build/venv/bin/tx push --source
	.build/venv/bin/tx push --translations --force --no-interactive

.build/locale/%/LC_MESSAGES/c2corg_ui-client.po: .tx/config
